...
| 界面 | 桌面行为 | 移动端行为 |
|---|---|---|
| 侧边栏 | 网格列，264–420px | 完全隐藏；左上角 ☰ 悬浮按钮（body 级 DOM）唤出**浮层抽屉**（`min(320px, 88vw)`），压在蒙层之上；☰ 右侧齿轮按钮**不打开抽屉也能进设置**；点会话/搜索结果/新建会话等**导航行**自动收起；搜索、设置、工具等其余交互保持打开；**有官方浮层（设置面板/目录选择器/popover 菜单，即 `aria-modal="true"` 或 `role="menu"`）打开时，浮层内的任何交互都不会动抽屉**；遮罩/收起按钮/Esc 收起 |
| 对话区 | 中央列 | **真全宽**——网格重写为单一 `minmax(0, 1fr)` 轨道，不再预留窄轨 |
| 头部 | 固定标题行 | **重排**：左侧为 ☰ 和设置齿轮预留槽位、标题簇紧凑化、工具区可横向滚动 |
