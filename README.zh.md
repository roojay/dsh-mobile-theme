...
| 界面 | 桌面行为 | 移动端行为 |
|---|---|---|
| 侧边栏 | 网格列，264–420px | 完全隐藏；左上角 ☰ 唤出浮层抽屉（`min(320px, 88vw)`）；设置从抽屉底部进入，宽度与菜单一致；点会话/搜索结果/新建会话等导航行收起；搜索、设置、工具保持打开 |
| 对话区 | 中央列 | **真全宽**——网格重写为单一 `minmax(0, 1fr)` 轨道，不再预留窄轨 |
| 头部 | 固定标题行 | 左侧为 ☰ 预留槽位、标题簇紧凑、工具区可横滚 |
...
| 设置 | 居中 800px 弹窗 + 侧边导航 | 铺满已展开的抽屉（与菜单同宽）+ 顶部横向分区导航 |
