...
| 界面 | 桌面行为 | 移动端行为 |
|---|---|---|
| 侧边栏 | 网格列，264–420px | 完全隐藏；左上角 ☰ 悬浮按钮（body 级 DOM）唤出**浮层抽屉**（`min(320px, 88vw)`），压在蒙层之上；右上角齿轮按钮**不打开抽屉也能进设置**；点会话/搜索结果/新建会话等**导航行**自动收起；搜索、设置、工具等其余交互保持打开；**有官方浮层（设置面板/目录选择器/popover 菜单，即 `aria-modal="true"` 或 `role="menu"`）打开时，浮层内的任何交互都不会动抽屉**；遮罩/收起按钮/Esc 收起 |
| 对话区 | 中央列 | **真全宽**——网格重写为单一 `minmax(0, 1fr)` 轨道，不再预留窄轨 |
| 头部 | 固定标题行 | **重排**：左侧为 ☰ 预留槽位、右侧为设置齿轮预留槽位、标题簇紧凑化、工具区可横向滚动 |
...
| 消息操作 | 28px | 32px 触控目标；**用户消息的时间常显**（短文本不占空间）；助手消息的时间/统计标签**默认隐藏、轻点显示（4 秒自动隐藏）**——hover 的触摸等价物；显示时上限 30% 宽 + 省略号，单行不溢出 |
...
| 无障碍 | — | ☰ 按钮经 frame MutationObserver 同步 `aria-expanded`；两个悬浮按钮的 `aria-label` 都跟随应用 `html[lang]` |
...
| 2 · 结构自标记 | 全宽网格、浮层抽屉、蒙层、☰ / 设置悬浮按钮、键盘抬升 | `data-slot` 结构 + DOM 列顺序，运行时自校验 | **优雅降级**：发现失败 → 不打标记 → 布局规则全部失效 → 回到官方窄轨布局、功能完好；`body[data-dsh-mobile-layout="degraded"]` + 一行控制台提示 |
...
- **抽屉依赖 CSS 与 JS 协同**：手机上窄轨完全隐藏，抽屉入口是注入的 ☰ 悬浮按钮；蒙层/Escape 关闭与按钮都经 `ctx.layout`。设置齿轮点击官方 `sidebar.settings` 触发器（`button[aria-haspopup="dialog"]`）。0.1.2 的设置面板是侧栏列里的 `position:fixed` 子节点，父级 `transform` 会把它困在屏外抽屉里，所以样式表在模态对话框打开时去掉该 `transform`。没有 layout 服务时 CSS 层仍生效，但不会注入悬浮按钮（手机上抽屉保持隐藏）。
